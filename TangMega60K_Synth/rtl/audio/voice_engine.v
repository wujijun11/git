...
// Clock: 49.152 MHz, nominal Fs=48 kHz. One frame consumes 9*N + overhead
...
module voice_engine #(
    ...
    localparam INIT=0, IDLE=1, CMD_READ=2, CMD_WRITE=3, PREP=4, LATCH=5,
        ...
        MASTER=16, FX_SEND=17, OUTPUT_WAIT=18;
    reg [4:0] state;
    ...
    reg [100:0] ram_write_data;
    voice_state_ram #(.VOICE_COUNT(VOICE_COUNT)) u_state (
    ...
    adsr #(.SAMPLE_RATE(SAMPLE_RATE),.ATTACK_MS(ATTACK_MS),
        .DECAY_MS(DECAY_MS),.RELEASE_MS(RELEASE_MS)) u_adsr (
    ...
    wavetable_rom #(.WAVE_FILE({ROM_DIR,"waves.hex"}),.NOTE_FILE({ROM_DIR,"midi.hex"}),
        .VELOCITY_FILE({ROM_DIR,"velocity.hex"})) u_tables (
    ...
    stereo_mixer #(.PAN_SPREAD(PAN_SPREAD)) u_mixer (
        ...
        .voice_id(voice_id),.sample_in(voice_sample),.left_sum(mix_left),.right_sum(mix_right)
    );
    ...
    always @(*) begin
        ...
        if(state==INIT) begin ram_write=1; ram_write_addr=init_id; end
            ...
            if(pending_on) begin
                ...
                ram_write_data={1'b1,pending_timbre,pending_velocity,3'd1,
            ...
            ram_write_data={w_active && !w_finished,w_timbre,w_velocity,w_stage,w_env,w_base,phase_next};
        end
    end
    always @(posedge clk or negedge rst_n) begin
        if(!rst_n) begin
            ...
            case(state)
                ...
                V_ROM: state<=V_ENV;
                V_ENV: begin
                    env_product<=$signed(wave_sample)*$signed({1'b0,w_env[23:8]});
                    state<=V_VEL;
                end
                V_VEL: begin
