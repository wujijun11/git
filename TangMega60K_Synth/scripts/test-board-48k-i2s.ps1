...
try {
    ...
    [xml]$projectXml=Get-Content clock48/BoardAudio48kI2S.gprj
    ...
    & C:/iverilog/bin/iverilog.exe -g2012 -DSIM -s tb_board_audio_48k_i2s `
        -o sim/board_audio_48k_i2s_test.vvp @sources $vendorModel sim/tb_board_audio_48k_i2s.sv
    if($LASTEXITCODE -ne 0) { throw '48 kHz external I2S compilation failed' }
    & C:/iverilog/bin/vvp.exe sim/board_audio_48k_i2s_test.vvp
    if($LASTEXITCODE -ne 0) { throw '48 kHz external I2S simulation failed' }
